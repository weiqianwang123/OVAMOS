...
class ValueMap(BaseMap):
    ...
    def __init__(
        ...
        self._update_counts = np.zeros((size, size), np.int32)
        ...
        if RECORDING:
            ...
            with open(JSON_PATH, "w") as f:
                f.write("{}")
    def remove_target_channel(self, target_index):
        """
        移除指定目标物品的通道，动态减少 `value_channels` 数量。

        Args:
            target_index (int): 要删除的目标通道索引
        """
        if self._value_channels <= 1:
            print("无法删除通道，最少需要 1 个通道！")
            return

        if target_index < 0 or target_index >= self._value_channels:
            print(f"无效索引 {target_index}，跳过删除")
            return

        # **删除指定通道**
        self._value_map = np.delete(self._value_map, target_index, axis=2)
        self._value_channels -= 1  # **更新通道数**
        print(f"✅  目标 {target_index} 已找到，删除该通道。当前通道数: {self._value_channels}")
    def reset(self) -> None:
        super().reset()
    ...
    def update_map(
        ...
        if if_decay:
            update_mask = curr_map > 0

            # 更新 _update_counts
            self._update_counts[update_mask] += 1  # **累积该像素的更新次数**
            update_counts = self._update_counts[update_mask]

            # 计算衰减权重
            decay_weights = 1 / (1 + np.exp((update_counts - decay_thresh) / deacy_speed))  # 10 是偏移量，2 控制衰减速度
            decay_weights = np.expand_dims(decay_weights, axis=-1)  # 变成 (H, W, 1)
            decay_weights = np.repeat(decay_weights, self._value_channels, axis=-1)  # 变成 (H, W, C)


            # **更新 value_map**
            self._value_map[update_mask] = (self._value_map[update_mask] * decay_weights) + \
                                        (values * (1 - decay_weights))
