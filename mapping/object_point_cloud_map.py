...
class ObjectPointCloudMap:
    ...
    def remove_target_class(self, target_class: str) -> None:
        """
        ...
        if target_class in self.clouds:
            del self.clouds[target_class]
            print(f"目标 {target_class} 已找到，从 `clouds` 中删除！")
        else:
            print(f"目标 {target_class} 不在 `clouds` 中，无需删除。")
        self.last_target_coord = None
    def update_map(
        self,
        object_name: str,
    ...
    def get_best_object(self, target_class: str, curr_position: np.ndarray) -> np.ndarray:
        ...
        return self.last_target_coord
    def get_best_object_among_targets(
        self,
        ...
        """
        # 先遍历所有目标对象，找出“全局最优”的那个点（离 curr_position 最近）
        best_dist = float("inf")
        best_point = None
        for target_class in target_classes:
            if self.has_object(target_class):
                target_cloud = self.get_target_cloud(target_class)
                # 如果没有任何点可用，就跳过
                if target_cloud is None or len(target_cloud) == 0:
                    continue

                # 找到这个 target_class 的最近点
                this_closest_point_2d = self._get_closest_point(target_cloud, curr_position)[:2]
                dist = np.linalg.norm(curr_position[:2] - this_closest_point_2d)

                # 更新全局最优点
                if dist < best_dist:
                    best_dist = dist
                    best_point = this_closest_point_2d

        # 如果所有目标对象都没有点云，则返回 None
        if best_point is None:
            return None

        # 接下来执行原先的“是否更新 self.last_target_coord” 的逻辑
        if self.last_target_coord is None:
            # 第一次找到目标，就把它记录为 last_target_coord
            self.last_target_coord = best_point
        else:
            # 根据差异距离来决定是否更新
            delta_dist = np.linalg.norm(best_point - self.last_target_coord)

            if delta_dist < 0.1:
                # 新点和上一个记录非常接近，不用更新
                return self.last_target_coord
            elif delta_dist < 0.5 and np.linalg.norm(curr_position - best_point) > 2.0:
                # 新点略有不同，但机器人离这个点太远，这点差异意义不大，不更新
                return self.last_target_coord
            else:
                # 否则更新记录
                self.last_target_coord = best_point
    ...
    def get_best_object_among_targets_reality(
        self,
        target_classes: List[str],
        curr_position: np.ndarray
        ) -> Optional[np.ndarray]:
        ...
        if self.last_target_coord is None:
            # 第一次找到目标，就把它记录为 last_target_coord
            self.last_target_coord = best_point
            self.last_target_class = best_class
        else:
            # 根据差异距离来决定是否更新
            delta_dist = np.linalg.norm(best_point - self.last_target_coord)
            ...
            if delta_dist < 0.1:
                # 新点和上一个记录非常接近，不用更新
                return self.last_target_coord,self.last_target_class
            elif delta_dist < 0.5 and np.linalg.norm(curr_position - best_point) > 2.0:
                # 新点略有不同，但机器人离这个点太远，这点差异意义不大，不更新
                return self.last_target_coord,self.last_target_class
            else:
