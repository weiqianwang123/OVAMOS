...
class ObstacleMap(BaseMap):
    ...
    def update_map_direct(self,new_map,height,width,tf_camera_to_episodic,origin_x,origin_y):
        agent_xy_location = tf_camera_to_episodic[:2, 3]
        agent_pixel_location = self._xy_to_px(agent_xy_location.reshape(1, 2))[0]
        # Explored: non-negative
        scaled_map =  np.full((1000,1000),-1,dtype=np.int8)
        new_origin_x = -25
        new_origin_y = -25
        delta_x = int((origin_x -new_origin_x)/0.05) 
        delta_y = int((origin_y -new_origin_y)/0.05) 
        for y in range(height):
            for x in range(width):
                ny = delta_y+y
                nx = delta_x+x
                if 0<=ny<1000 and 0<=nx<1000:
                    scaled_map[ny][nx] = new_map[y][x]
        
        scaled_map = np.rot90(scaled_map, k=-1).copy()

        explored = scaled_map >= 0
        self.explored_area = explored.astype(np.int8)
        # self.explored_area[self.explored_area > 0] = 1

        # Obstacles: value >= 50
        obstacles = scaled_map >= 50
        self._map = obstacles
        ...
        if len(contours) > 1:
            ...
            new_area = np.zeros_like(self.explored_area, dtype=np.uint8)
            cv2.drawContours(new_area, contours, best_idx, 1, -1)  # type: ignore
            # Erode (shrink) the explored area slightly
            # kernel = np.ones((3, 3), np.uint8)  # 3x3 kernel, you can adjust
            # new_area = cv2.erode(new_area, kernel, iterations=3)
