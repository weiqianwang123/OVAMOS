...
class POMDP_BASIC_Policy():
    ...
    def act(self,
        ...
        if not self._done_initializing:  # Initialize
            ...
            best_frontier =  self._pomdp_planner.get_best_frontier()
            action = self._pointnav(np.array(best_frontier),stop=False).item()
        else:
            mode = "navigate"
            if not self._done_initialize_pomdp:
                self._initialize_pomdp()
        ...
        print(f"Step: {self._num_steps} | Mode: {mode} | Action: {action}")
    ...
    def _get_target_object_location(self, position: np.ndarray) -> Union[None, np.ndarray]:
        ...
        return self._object_map.get_best_object_among_targets(target_objects_list,position)
